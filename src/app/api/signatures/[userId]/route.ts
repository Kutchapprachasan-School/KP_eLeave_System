...
import { verifyAndConsumeSignatureToken } from "@/lib/signature-token";
...
export async function GET(
  ...
  // Dual-Auth Guard: Either Single-Use Scoped HMAC Token OR Active User Session
  if (tokenParam) {
    const isTokenValid = await verifyAndConsumeSignatureToken(
      tokenParam,
      docIdParam,
      userId,
      "PRINT_FACILITY_A4"
    ...
    if (!isTokenValid) {
      return new NextResponse("Unauthorized: Invalid, expired, or already consumed signature token", { status: 401 });
    }
  } else {
    // Session Cookie Authentication for interactive UI
