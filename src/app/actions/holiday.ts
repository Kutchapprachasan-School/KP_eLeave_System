...
async function fetchHolidaysDirect(yearBE?: number) {
  const where: any = {};
  if (yearBE) {
...
// 24-Hour Server Cache for Holidays (On-demand revalidation on edit/create/import)
export async function getHolidays(yearBE?: number) {
  return unstable_cache(
    async () => fetchHolidaysDirect(yearBE),
    [`holidays-cache-${yearBE || "all"}`],
...
export async function createHoliday(startDateStr: string, endDateStr: string, name: string, isWorkday: boolean = false) {
  ...
  try { revalidateTag("holidays"); } catch (e) {}
