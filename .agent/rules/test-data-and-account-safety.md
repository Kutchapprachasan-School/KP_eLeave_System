...
> **"ฐานข้อมูล Production ต้องมีเฉพาะบุคลากรจริง 76 ท่านเท่านั้น ห้ามมีบัญชีทดสอบหลงเหลือเด็ดขาด"**
...
1. **ห้ามสร้างบัญชีผู้ใช้ทดสอบทิ้งไว้ในตาราง `User`:**
   ...
   - ตรวจสอบยอดบุคลากรจริงของโรงเรียน: ต้องคงที่อยู่ที่ **76 คน** (บุคลากรจริงที่สร้างในเดือน ก.ค. 2569) เสมอ
...
ก่อนปิดงานหรือส่งมอบงานให้ผู้ใช้ทุกครั้ง AI Agent ต้องตรวจสอบยอดผู้ใช้ในฐานข้อมูลด้วยคำสั่ง:

```sql
-- ต้องได้ผลลัพธ์ 0 แถวเสมอ
SELECT id, email, name FROM "User" 
WHERE "createdAt" >= '2026-09-14 00:00:00' 
   OR email LIKE '%test%' 
   OR name LIKE '%ทดสอบ%';

-- ยอดรวมบุคลากรจริงต้องเท่ากับ 76 คนเสมอ
